...
export default function SubmissionDetail() {
  ...
  return (
    <div className={styles.dashboardWrapper} style={{ maxWidth: 900 }}>
      {/* Back button + header */}
      <header className={styles.header} style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <button
          ...
          style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: '50%', width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
        >
          <ChevronLeft size={20} />
        </button>
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: '1.7rem', fontWeight: 800, color: 'var(--text-main)', margin: 0 }}>
            {sub.propertyTitle || 'Unnamed Property'}
          </h1>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 4, padding: '3px 12px', borderRadius: 20, background: cfg.badge, color: cfg.text, fontSize: '0.8rem', fontWeight: 700, textTransform: 'capitalize' }}>
