  ...
  return (
    <div className={styles.dashboardWrapper}>
      ...
      <header className={styles.header} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '2rem', fontWeight: 800, color: 'var(--text-main)' }}>Seller Dashboard</h1>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Manage your property listings and agreements.</p>
        </div>
        <button
          onClick={() => navigate('/agreements/list')}
          style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#ed1b24', color: 'white', padding: '0.8rem 1.5rem', borderRadius: '12px', fontWeight: 600, border: 'none', cursor: 'pointer', boxShadow: '0 4px 12px rgba(237,27,36,0.25)' }}
        >
          <Plus size={20} /> Sell Your Property
        </button>
      </header>
      ...
      <div className={styles.statsRow}>
        <div className={styles.statCard}>
          ...
          <div className={styles.statInfo}><label>Properties Submitted</label><span>{submissions.length}</span></div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statIcon} style={{ background: 'rgba(59, 130, 246, 0.15)', color: '#3b82f6' }}><Clock size={20} /></div>
          <div className={styles.statInfo}><label>Pending Approval</label><span>{submissions.filter(s => s.status === 'pending').length}</span></div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statIcon} style={{ background: 'rgba(16, 185, 129, 0.15)', color: '#10b981' }}><CheckCircle2 size={20} /></div>
          <div className={styles.statInfo}><label>Approved Listings</label><span>{submissions.filter(s => s.status === 'approved').length}</span></div>
        </div>
      </div>
      ...
      <div className={styles.mainCard}>
        ...
        <div className={styles.content}>
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div key="loading" className={styles.loader}>
